...
//                    "jdbc:mysql://localhost:3306/IsolationDB?useSSL=false&allowPublicKeyRetrieval=true", "root", "P@ssW0rd");
    ...
    public void GetActivityPlaces(){
        ...
        try{
            ...
            while(rs.next())
                place = new ActivityPlace(rs.getNString(1), rs.getInt(2),rs.getInt(3));
            System.out.println("Activity name:" + place.getPlaceName()+"  "+ "Actual guests:" + place.getActualGuest() + " " + "Maximum guests" + place.getMaxGuest());
            con.close();
